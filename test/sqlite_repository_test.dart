...
void main() {
  group('sqlite_repo_test', () {
    ...
    test("should test item", () async {
      int id =
          await repo.addItem(Item(name: "item1", currency: "USD", price: 1.0));
      expect(id, 1);
      var id2 =
          await repo.addItem(Item(name: "item2", currency: "USD", price: 1.0));
      expect(id2, 2);
    ...
    test("should test assets", () async {
      ...
      List<int>? assetData = (await repo.fetchAssetByURL(url))!;
    ...
    test(
        "fetchSaleByPaymentHash when paymentHash is unknown should return null",
        () async {
      const hash = "a_hash";
    ...
    test("fetchSaleByPaymentHash when paymentHash is valid should return sale",
        () async {
      const hash = "a_hash";
      await repo.addSale(
        Sale(
