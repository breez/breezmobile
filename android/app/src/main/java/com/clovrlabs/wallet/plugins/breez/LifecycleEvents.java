...
import com.clovrlabs.wallet.ClovrApplication;
...
public class LifecycleEvents implements StreamHandler, FlutterPlugin, ActivityAware, LifecycleObserver {
    ...
    public void onResume() {
        ClovrApplication.isBackground = false;
        Log.d("Clovr", "App Resumed - OnPostResume called");
    ...
    public void onStop() {
        ClovrApplication.isBackground = true;
        Log.d("Clovr", "App Paused - onPause called");
        if (m_eventsListener != null) {
            _executor.execute(() -> {
                binding.getActivity().runOnUiThread(() -> {
