...
final _log = FimberLog("GraphDownloader");
...
class GraphDownloader {
  ...
  Future init() async {    
    downloadManager.downloadProgress.listen((event) async {
      _log.v("GraphDownloader event: ${event.id}");
      _log.v("GraphDownloader event: ${event.status}");
      _log.v("GraphDownloader event: ${event.percentage}");
      ...
      if (currentTask != null &&
        ...
        _log.v("GraphDownloader task status = ${currentTask.status}");
    ...
    for (var i = 0; i < tasks.length; ++i) {
      if (tasks[i].url == downloadURL) {
        ...
        if (tasks[i].status == DownloadTaskStatus.enqueued) {
          _log.v("removing enqueued download graph task");
          downloadManager.removeTask(tasks[i].taskId);
          continue;
        }

        if (tasks[i].status == DownloadTaskStatus.complete) {
          _log.v("Already has a recently completed graph download task, using it");
          _onTaskFinished(tasks[i]);
          return _downloadCompleter.future;
        }

        if (tasks[i].status == DownloadTaskStatus.running) {
          _log.v("Already has graph download task running, not starting another one");
          return _downloadCompleter.future;
        }
      }
    }

    _log.v("Graph download started");
