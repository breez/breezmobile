...
StreamBuilder<BreezUserModel> _buildDrawerHeaderContent(UserProfileBloc user, bool drawAvatar) {
  return StreamBuilder<BreezUserModel>(
      ...
      builder: (context, snapshot) {
        if (!snapshot.hasData) {
          ...
          List<Widget> drawerHeaderContent = new List<Widget>();
          drawerHeaderContent.add(_buildThemeSwitch(snapshot, user, context));
          if (drawAvatar) {
            drawerHeaderContent..add(_buildAvatarButton(snapshot))..add(_buildBottomRow(snapshot, context));
          }
...
GestureDetector _buildThemeSwitch(AsyncSnapshot<BreezUserModel> snapshot, UserProfileBloc user, BuildContext context) {
  return GestureDetector(
...
Row _buildBottomRow(AsyncSnapshot<BreezUserModel> snapshot, BuildContext context) {
  return Row(
