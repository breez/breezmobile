...
class LoungePaymentsBloc with AsyncActionsHandler {
  ...
  Future _payBoost(PayBoost action) async {
    if (action.nodeID.isEmpty) {
      throw new Exception("must enter a lounge to pay boost");
    }
    _paymentEventsController.add(LoungePaymentEvent(
        LoungePaymentEventType.BoostStarted, action.sats, action.nodeID));
    _payLounge(action.title, "", action.nodeID, action.sats, boost: true);
  }
    ...
    withBreez.forEach((d) async {
      ...
      if (netPay > 0 && amount <= total && maxFee > 0) {
        ...
        _breezLib
          ...
          log.info("succesfully paid $netPay to lounge destination ${d.address}");
